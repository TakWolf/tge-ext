...
impl ResolutionAdaptParams {
    pub fn convert_to_canvas_position(&self, window_position: impl Into<LogicalPosition>) -> Position<f32> {
        ...
        Position::new(
            (window_position.x - self.viewport.x) / self.scale_factor.x,
            (window_position.y - self.viewport.y) / self.scale_factor.y,
        )
    }
    ...
    pub fn convert_to_window_position(&self, canvas_position: impl Into<Position<f32>>) -> LogicalPosition {
        ...
        Position::new(
            canvas_position.x * self.scale_factor.x + self.viewport.x,
            canvas_position.y * self.scale_factor.y + self.viewport.y,
        )
    }
}
